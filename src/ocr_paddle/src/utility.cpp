...
void Utility::VisualizeBboxes(const cv::Mat &srcimg,
  ...
  for (int n = 0; n < ocr_result.size(); n++) {
    ...
    cv::circle(img_vis, rook_points[0], 5, cv::Scalar(0, 0, 255), -1);
    cv::circle(img_vis, rook_points[1], 5, cv::Scalar(0, 255, 0), -1);
    cv::circle(img_vis, rook_points[2], 5, cv::Scalar(255,0, 0), -1);
